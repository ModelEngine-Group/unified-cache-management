...
static void decompress_fp8_to_bf16_inplace (
    uint8_t *p_data,
    size_t   n_bf16
) {
...
static int decompress_tunstall_trunc (
    uint16_t      *p_dst,
    const uint8_t *p_src,
    size_t         n_bf16
) {
...
static int compact_tunstall_inplace_streams (
    uint8_t  *p_stream_end,
    uint8_t **pp_sm,
    uint8_t **pp_sm_end,
    uint8_t **pp_mark,
    uint8_t **pp_mark_end,
...
static int spill_tunstall_inplace_streams (
    uint8_t  *p_tail,
    size_t    tail_size,
    uint8_t **pp_sm,
    uint8_t **pp_sm_end,
...
static int decompress_tunstall_trunc_inplace (
    uint8_t *p_data,
    size_t   n_bf16
) {
    RET_ERROR_IF(R_ERR_SYNTAX, (n_bf16 % 32 != 0));
    ...
    for (size_t i_block = 0; i_block < n_bf16; i_block += BLOCK_SIZE) {
        ...
        for (const uint8_t *p_exp_read = buf_exp; p_exp_read < p_exp_end; p_exp_read += 32) {
            ...
            if (!using_tail && (size_t)(p_sm_end - p_mark) <= sizeof(tail)) {
                err = spill_tunstall_inplace_streams(
                    tail, sizeof(tail),
                    &p_sm, &p_sm_end, &p_mark, &p_mark_end, &p_extra, &p_extra_end
                );
                if (err) return err;
                using_tail = 1;
            }
            ...
            if (!using_tail) {
                if ((size_t)(p_sm_end - p_mark) <= sizeof(tail)) {
                    ...
                    uint8_t *p_first_src = (p_mark < p_mark_end) ? p_mark : ((p_extra < p_extra_end) ? p_extra : p_sm);
                    if (p_dst + dst_block_bytes > p_first_src) {
                        err = compact_tunstall_inplace_streams(
                            p_sm_end,
                            &p_sm, &p_sm_end, &p_mark, &p_mark_end, &p_extra, &p_extra_end
                        );
                        if (err) return err;
                        p_first_src = (p_mark < p_mark_end) ? p_mark : ((p_extra < p_extra_end) ? p_extra : p_sm);
                ...
                for (size_t lane = 0; lane < 8; lane++) {
                    ...
                    p_bf16[lane]      = ( (sm_lo       & 0x08) << 12) | ((exp_0  + BF16_EXP_MIN) << 7) | ( (sm_lo       & 0x07) << 4);
                    p_bf16[lane + 8]  = (((sm_lo >> 4) & 0x08) << 12) | ((exp_8  + BF16_EXP_MIN) << 7) | (((sm_lo >> 4) & 0x07) << 4);
                    p_bf16[lane + 16] = ( (sm_hi       & 0x08) << 12) | ((exp_16 + BF16_EXP_MIN) << 7) | ( (sm_hi       & 0x07) << 4);
                    p_bf16[lane + 24] = (((sm_hi >> 4) & 0x08) << 12) | ((exp_24 + BF16_EXP_MIN) << 7) | (((sm_hi >> 4) & 0x07) << 4);
                ...
                if (ENABLE_EXTRA_DECOMPRESSION && has_extra) {
                    bf16_0_7   = vorrq_u16(bf16_0_7  , vandq_u16(vshlq_u16(vdupq_n_u16(extra[0]), v_extra_shift), v0008));
                    bf16_8_15  = vorrq_u16(bf16_8_15 , vandq_u16(vshlq_u16(vdupq_n_u16(extra[1]), v_extra_shift), v0008));
                    bf16_16_23 = vorrq_u16(bf16_16_23, vandq_u16(vshlq_u16(vdupq_n_u16(extra[2]), v_extra_shift), v0008));
                    bf16_24_31 = vorrq_u16(bf16_24_31, vandq_u16(vshlq_u16(vdupq_n_u16(extra[3]), v_extra_shift), v0008));
                    bf16_0_7   = vorrq_u16(bf16_0_7  , v0003);
                    bf16_8_15  = vorrq_u16(bf16_8_15 , v0003);
                    bf16_16_23 = vorrq_u16(bf16_16_23, v0003);
                    bf16_24_31 = vorrq_u16(bf16_24_31, v0003);
