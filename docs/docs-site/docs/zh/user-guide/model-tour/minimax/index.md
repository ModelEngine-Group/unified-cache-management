...
=== "H100"
    ...
    将 `MODEL_DIR` 设为宿主机上存放 `MiniMax-M2.7` 的父目录（示例模型路径为 `/mnt/model-2/MiniMax-M2.7`），从[快速开始](../../quick_start/index.md#vllm)复制支持该模型和 H100 的 CUDA **UCM 镜像**地址到 `IMAGE`，在**宿主机**执行：
