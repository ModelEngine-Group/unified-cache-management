...
class GSAPrefetchBase:
    ...
    def deal_async_prefetch(
        ...
        if (
            self.prefetch_engine_c.get_prefetch_status() and
            self.ptopk_prefetch_enable and
            self.is_topk_update
        ):
        ...
        for index, req_id in enumerate(self.req_ids_bs):
            ...
            if gsa_metadata.gsa_stats[req_id].reamin_map != None:
                ...
                for layer_id in range(self.num_attention_layers):
                    topk_block_list = sorted(list(gsa_metadata.gsa_stats[req_id].reamin_map[layer_id].values()))
                    prefetch_blocks_list = list(gsa_metadata.gsa_stats[req_id].prefetch_map[layer_id].values())
                ...
                prefetch_block_tensor = torch.tensor(prefetch_blocks_list_all, dtype=torch.int32)
            else:
                ...
                prefetch_block_tensor = torch.tensor(prefetch_blocks_list, dtype=torch.int32)
            ...
            if self.is_gsa_req_id[req_id]:
                if gsa_metadata.gsa_stats[req_id].reamin_map != None:
                    ...
                    self.prefetch_engine_c.set_blocks_map(
                        req_id, block_table_list, prefetch_idx + remain_index, 
                        gsa_metadata.gsa_stats[req_id].block_hashes, max_idx
                        )
