...
path = os.getcwd()
sys.path.append(os.path.join(path, 'unifiedcache/csrc/ucmprefetch'))
...
class GSAPrefetchBase:
    ...
    def _init_tensor(self):
        ...
        self.use_block_table = torch.zeros((self.num_attention_layers, self.max_bs, int(self.sp_max_len)),
                                         ...
                                         pin_memory=False,
                                         device=self.device_config.device_type)
        self.use_block_table_cpu = torch.zeros((self.num_attention_layers, self.max_bs, int(self.sp_max_len)),
                                         dtype=torch.int32,
                                         pin_memory=is_pin_memory_available(),
                                         device="cpu") * -1
    ...
    def _first_topk_deal(
        ...
        for index, req_id in enumerate(self.req_ids_bs):
            if gsa_metadata.gsa_stats[req_id].remain_idx != None:
                ...
                self.free_block_len[:, bs_index] = len(prefetch_idx)
        ...
        for index, req_id in enumerate(self.req_ids_bs):
            ...
            if gsa_metadata.gsa_stats[req_id].stage() == SequenceStage.DECODE:
                if free_slot == 0:
                    ...
                    self.gsa_seq_len[:, bs_index].add_(-1).mul_(self.block_size).add_(free_slot)
            else:
                self.block_map_flag[req_id] = []
                self.block_table_flag[req_id] = []
            ...
            if req_id in self.block_map_flag:
                for block_mp_add in self.block_map_flag[req_id]:
                    ...
                    if gsa_metadata.gsa_stats[req_id].topk_buf_tmp != None:
                        if not torch.isin(torch.tensor([block_mp_add[0]]),
                                        gsa_metadata.gsa_stats[req_id].topk_buf_tmp[0]).item():
                            gsa_metadata.gsa_stats[req_id].topk_buf_tmp = torch.nn.functional.pad(gsa_metadata.gsa_stats[req_id].topk_buf_tmp,
                                                                                                (0, 1), value=block_mp_add[0])
            ...
            if self.is_gsa_req_id[req_id] and gsa_metadata.gsa_stats[req_id].topk_buf_tmp != None:
                ...
                for layerid in range(self.num_attention_layers):
                    ...
                    if remain_slot == 0:
                        ...
                        gen_len_tmp[layerid][index].add_(one_topk_len * self.block_size - self.block_size + remain_slot)
            else:
                block_table_tmp[:, index, :len(self.block_table_list_bs[index])] = one_block_table
                gen_len_tmp[:, index] = gsa_metadata.gsa_stats[req_id].get_seq_len()
        return block_table_tmp, gen_len_tmp
    ...
    def model_input_del(
        ...
        if self.atb_gsa_enable:
            ...
            gsa_model_input["is_topk"] = self.num_token % 6 == 0
            ...
            if self.is_topk_cal:
                ...
                for index, req_id in enumerate(self.req_ids_bs):
                    ...
                    one_topk_len = min(self.topk_len, one_block_len)
        ...
        # if self.atb_gsa_enable and self.ptopk_prefetch_enable:
        #     self.prefetch_engine_c.SetIsAtbDone(False)
        
    def block_hash(self, request_id, block_id):
    ...
    def deal_async_prefetch(
        ...
        # if self.atb_gsa_enable and self.ptopk_prefetch_enable:
        #     self.prefetch_engine_c.SetIsAtbDone(True)
        
        if self.atb_gsa_enable:
            if self.ptopk_prefetch_enable:
                if self.is_prefetch_flag:
