...
class PrefillMLAPreprocessResult(NamedTuple):
    ...
    gsa_kpe: Optional[torch.Tensor] = None
    gsa_knope: Optional[torch.Tensor] = None
...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def _mla_preprocess(self, layer_name, hidden_states, kv_cache,
        ...
        if has_prefill:
            ...
            gsa_kpe = prefill_k_pe
            gsa_knope = prefill_k_c_normed
            ...
            prefill_preprocess_res = PrefillMLAPreprocessResult(
                ...
                prefill_value, gsa_kpe, gsa_knope)
    ...
    def forward(
        ...
        if prefill_preprocess_res is not None:
            ...
            query, key, value, sp_out = maybe_execute_sparse_attention_begin(torch.cat([prefill_preprocess_res.q_nope, prefill_preprocess_res.q_pe], dim=-1),
                                                                             prefill_preprocess_res.gsa_knope,
                                                                             prefill_preprocess_res.gsa_kpe,
                                                                             layer_name,
                                                                             forward_context,
                                                                             phase="prefill",
                                                                             k_hash=k_hash)
