...
* **`local_hostname`** *(required)*:  
  ...
  **Replace `"127.0.0.1"` with the actual IP of this node.**
...
* **`storage_backends`** *(required for `Mooncake|Posix`)*:  
  ...
  **Replace `"/mnt/test"` with your actual storage path.**
...
**Replace `141.111.32.88` (master node IP), `141.111.32.82` (worker node IP), the model path, and the UCM config file path with your actual values.**
