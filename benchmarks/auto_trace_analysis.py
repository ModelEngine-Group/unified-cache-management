...
class EvictionStrategy(ABC):
    """Owns the ordered data structure and controls eviction order.

    Implementations manage their own storage (OrderedDict, heap, etc.).
    The pool delegates all ordering decisions here — adding a new policy
    means writing a new subclass, not modifying the pool.
    """
    ...
    def add(self, key, size: int) -> None:
        """Insert a new entry or update an existing one's size."""
        ...

    @abstractmethod
    def touch(self, key) -> None:
        """Called on access (get/peek hit) — refresh position if needed."""
        ...
    ...
    @abstractmethod
    def evict_one(self) -> tuple:
        """Pick and remove the next victim. Returns (key, size)."""
        ...


class LRUStrategy(EvictionStrategy):
    """Least-recently-used via OrderedDict: access moves to end, evict from head."""
...
class BytePool:
    """Byte-level pool for UCM DRAM/FS stores.

    Unlike BlockPool (block-count capacity), BytePool tracks bytes. Entries
    can have different sizes (FA ``fa_file_size`` vs WA ``wa_file_size``),
    so eviction is by byte budget, not entry count. Eviction order is
    delegated to an :class:`EvictionStrategy` (default LRU).
    """

    def __init__(self, capacity_bytes: int, strategy: EvictionStrategy | None = None):
    ...
    def get(self, key) -> bool:
        if self._capacity <= 0 or key not in self._strategy:
            return False
        self._strategy.touch(key)
        return True
    ...
    def put(self, key, size_bytes: int) -> None:
        if self._capacity <= 0 or size_bytes > self._capacity:
            return
        if key in self._strategy:
            self._used_bytes -= self._strategy.get_size(key)
        self._strategy.add(key, size_bytes)
        self._used_bytes += size_bytes
        while self._used_bytes > self._capacity and len(self._strategy) > 1:
            evict_key, evict_size = self._strategy.evict_one()
            self._used_bytes -= evict_size
            if evict_key == key:
                self._strategy.add(evict_key, evict_size)
                self._used_bytes += evict_size


@dataclass
class GroupContext:
    """Base: per-group runtime state common to all model types.

    FA groups (FAWA FA, Mamba FA) use this directly with chain-based
    hash derivation. WA and mamba subclasses override as needed.
    """
...
@dataclass
class FAWAGroupContext(GroupContext):
    """FAWA WA groups: sliding window sparse caching."""
...
@dataclass
class MambaGroupContext(GroupContext):
    """Mamba state groups: hash includes group_id + boundary prefix.

    No extra fields — only overrides derive_block_hash to produce
    per-group state hashes. Per-rank split handled externally by
    _rank_ids().
    """

    def derive_block_hash(self, block_idx: int, chain: list) -> object:
        prefix = chain[(block_idx + 1) * self.scale_factor - 1]
        boundary = (block_idx + 1) * self.logical_block_size
        return f"g{self.group_id}:B{boundary}:{prefix}"
...
class MambaContext(SimContext):
    ...
    mamba_groups: list[MambaGroupContext] = field(default_factory=list)
    gated_tokens: int = 0
    gated_tier: int = -1
...
class FAWAContext(SimContext):
    wa_dram: BytePool | None = None
    fa_groups: list[GroupContext] = field(default_factory=list)
    wa_groups: list[FAWAGroupContext] = field(default_factory=list)
...
class HitRateSimulator(ABC):
    ...
    # --- Pool helpers (moved from SimTopology) ---

    def _num_dram_pools(self) -> int:
        if self.topo.unified:
            return 1
        return self.topo.num_nodes if self.topo.is_mla else self.topo.dp_size

    def _select_pool(self, dp_rank: int, pools: list) -> Any:
        if self.topo.unified:
            return pools[0]
        if self.topo.is_mla:
            dp_per_node = (
                max(1, self.topo.dp_size // self.topo.num_nodes)
                if self.topo.num_nodes > 0
                else 1
            )
            node = min(dp_rank // dp_per_node, self.topo.num_nodes - 1)
            return pools[node]
        return pools[dp_rank]

    def _dram_per_pool_bytes(self, total_dram: int) -> int:
        if self.topo.unified:
            return total_dram if self.topo.is_mla else total_dram // self.topo.tp_size
        if self.topo.is_mla:
            return total_dram // self.topo.num_nodes
        return total_dram // (self.topo.dp_size * self.topo.tp_size)

    def _fs_capacity_bytes(self, total_fs: int) -> int:
        return total_fs if self.topo.is_mla else total_fs // self.topo.tp_size
    ...
    def _init_pools(self, dram_cap: int, fs_cap: int) -> None:
        ...
        dram_cap and fs_cap are total budgets; this method splits them
        into per-pool sizes based on topology (MLA/GQA/Unified).
        """
        per_pool = self._dram_per_pool_bytes(dram_cap)
        n = self._num_dram_pools()
        self.dram_pools = [BytePool(per_pool) for _ in range(n)]
        self.fs_pool = BytePool(self._fs_capacity_bytes(fs_cap))

    def _get_pools(self, dp_rank: int) -> tuple:
        """Default: (hbm, dram, fs) 3-tuple. FAWA overrides to 4-tuple."""
        hbm = self.hbm_pools[dp_rank]
        dram = self._select_pool(dp_rank, self.dram_pools)
    ...
    def _lookup(self, record: TraceRecord, ctx: SimContext) -> None:
        ...
        for i in range(ucm_prefix):
            ...
            if i < hbm_prefix:
                ...
                if ctx.fs.get(hash_id):
                    ctx.dram.put(hash_id, entry_size)
                else:
                    ctx.dram.get(hash_id)
    ...
    def _forward_sim(self, record: TraceRecord, ctx: SimContext) -> None:
        ...
        while pos < total:
            ...
            for i in range(fa_dumped, chunk_end):
                hash_id = record.hash_ids[i]
                ctx.dram.put(hash_id, entry_size)
                self.producer_map[hash_id] = ctx.req_idx
                ctx.fs.put(hash_id, entry_size)
                self.producer_map[hash_id] = ctx.req_idx
            fa_dumped = chunk_end
...
class MambaSimulator(HitRateSimulator):
    ...
    def _new_context(self, req_idx: int, dp_rank: int) -> MambaContext:
        hbm, dram, fs = self._get_pools(dp_rank)
        ctx = MambaContext(req_idx=req_idx, hbm=hbm, dram=dram, fs=fs)
        all_groups = self._build_group_contexts()
        ctx.fa_groups = [g for g in all_groups if g.manager_type != "mamba"]
        ctx.mamba_groups = [g for g in all_groups if g.manager_type == "mamba"]
        return ctx

    def _build_group_contexts(self) -> list[GroupContext]:
        lcm_block = self.topo.lcm_block_size
        vllm_bs = self.topo.vllm_hash_block_size
        n_mamba = self.topo.mamba_groups
        out: list[GroupContext] = [
            GroupContext(
                group_id=0,
                logical_block_size=lcm_block,
                vllm_hash_block_size=vllm_bs,
                manager_type="full_attention",
                alignment_tokens=lcm_block,
            )
        ]
    ...
    def _lookup(self, record: TraceRecord, ctx: MambaContext) -> None:
        ...
        vllm_bs = self.topo.vllm_hash_block_size
        total = len(chain)
        fa_group = ctx.fa_groups[0] if ctx.fa_groups else None
        step = lcm_block // vllm_bs if lcm_block > 0 and vllm_bs > 0 else 1
        num_boundaries = total // step if step > 0 else 0

        # 1a. FA forward
        hbm_prefix = 0
        fa_gid = fa_group.group_id if fa_group else 0
        if fa_group:
            for i in range(total):
                hash_id = chain[i]
                if ctx.hbm.peek(hash_id, fa_gid):
                    hbm_prefix = i + 1
                    ctx.hit_roots.add(
                        self.request_groups.find(
                            self.producer_map.get(hash_id, ctx.req_idx)
        ...
        hbm_gated = 0
        hbm_gated_tier = -1
        if lcm_block > 0 and hbm_prefix > 0 and ctx.mamba_groups:
            max_boundary = hbm_prefix * vllm_bs
            candidates = [
                (k + 1) * lcm_block
                for k in range(num_boundaries)
                if (k + 1) * lcm_block <= max_boundary
            ]
            for boundary in reversed(candidates):
                k = boundary // lcm_block - 1
                ok = True
                for g in ctx.mamba_groups:
                    rank0_hash = g.derive_block_hash(k, chain)
                    if not ctx.hbm.peek(rank0_hash, g.group_id):
                        ok = False
                        break
                if ok:
                    hbm_gated = boundary
                    ...
                    for g in ctx.mamba_groups:
                        ...
                        ctx.hit_roots.add(
                            self.request_groups.find(
                                self.producer_map.get(rank0_hash, ctx.req_idx)
                            )
                        )
                    break
        ...
        ucm_gated = 0
        ucm_gated_tier = -1
        if lcm_block > 0 and ucm_prefix > 0 and ctx.mamba_groups:
            ...
            for boundary in reversed(candidates):
                ...
                for g in ctx.mamba_groups:
                    rank0_hash = g.derive_block_hash(k, chain)
                    for cid in self._rank_ids(rank0_hash, tp, derive_mamba):
                        if ctx.dram.peek(cid):
        ...
        # Phase 3: UCM heat update + FS→DRAM promotion
        if lcm_block > 0:
            # FA entries (raw chain hash for DRAM/FS)
            for i in range(ucm_prefix):
                hash_id = chain[i]
                if i < hbm_prefix:
                    ctx.dram.get(hash_id)
    ...
    def _alloc_after_lookup(self, record: TraceRecord, ctx: MambaContext) -> None:
        ...
        for i in range(ctx.prefix_hit_count):
            hash_id = chain[i]
            bid = ctx.hbm.touch_get(hash_id, fa_gid)
            if bid is None:
                bid = self._alloc_and_cache(hash_id, fa_gid, ctx.hbm, ctx.req_idx)
            if bid is not None:
                ctx.fa_track.append((bid, True))

        # Mamba blocks at gated boundary (HBM: rank0 only, single block per group)
        if (
            ctx.gated_tokens > 0
            and ctx.mamba_groups
            and ctx.gated_tokens % lcm_block == 0
        ):
            k = ctx.gated_tokens // lcm_block - 1
            for g in ctx.mamba_groups:
                rank0_hash = g.derive_block_hash(k, chain)
                bid = ctx.hbm.touch_get(rank0_hash, g.group_id)
                if bid is None:
                    bid = self._alloc_and_cache(
                        rank0_hash, g.group_id, ctx.hbm, ctx.req_idx
                    )
                if bid is not None:
    ...
    def _forward_sim(self, record: TraceRecord, ctx: MambaContext) -> None:
        ...
        prev_mamba_running: list[tuple[int, bool]] = list(ctx.mamba_track)
        ...
        while pos < total:
            ...
            if is_lcm and chunk_token_end > ctx.gated_tokens:
                ...
                for g in ctx.mamba_groups:
                    ...
                    for cid in self._rank_ids(rank0_hash, tp, derive_mamba):
                        ctx.dram.put(cid, entry_size)
                        self.producer_map[cid] = ctx.req_idx
                        ctx.fs.put(cid, entry_size)
                        self.producer_map[cid] = ctx.req_idx
...
class FAWASimulator(HitRateSimulator):
    ...
    def _init_pools(self, dram_cap: int, fs_cap: int) -> None:
        ...
        n = self._num_dram_pools()
        if self.topo.unified:
            shared = [BytePool(per_pool) for _ in range(n)]
            self.fa_dram_pools = shared
            self.wa_dram_pools = shared
        else:
            half = per_pool // 2
            self.fa_dram_pools = [BytePool(half) for _ in range(n)]
            self.wa_dram_pools = [BytePool(half) for _ in range(n)]
        self.fs_pool = BytePool(self._fs_capacity_bytes(fs_cap))

    def _get_pools(self, dp_rank: int) -> tuple:
        hbm = self.hbm_pools[dp_rank]
        fa_dram = self._select_pool(dp_rank, self.fa_dram_pools)
        wa_dram = self._select_pool(dp_rank, self.wa_dram_pools)
        return hbm, fa_dram, wa_dram, self.fs_pool

    def _build_group_contexts(self) -> list[GroupContext]:
        at = self.topo.alignment_tokens or self.topo.vllm_hash_block_size
        out: list[GroupContext] = []
        for i, spec in enumerate(self.topo.group_specs):
            common = dict(
                group_id=i,
                logical_block_size=spec.logical_block_size,
                vllm_hash_block_size=self.topo.vllm_hash_block_size,
                manager_type=spec.manager_type,
                alignment_tokens=at,
            )
            if spec.manager_type == "sliding_window":
                out.append(
                    FAWAGroupContext(
                        **common,
                ...
                out.append(GroupContext(**common))
        return out
    ...
    def _new_context(self, req_idx: int, dp_rank: int) -> FAWAContext:
        ...
        group_contexts = self._build_group_contexts()
        fa_groups = [
            g
    ...
    def _lookup(self, record: TraceRecord, ctx: FAWAContext) -> None:
        ...
        # Phase 2: UCM peek
        ctx.ucm_prefix, ctx.fa_ext_tier = self._ucm_fa_lookup(
            chain,
        ...
        for ucm_idx in range(min(gated_ucm, num_ucm_blocks)):
            ...
            if is_hbm_hit:
                ...
                if ctx.fs.get(self._fa_key(ucm_hash)):
                    ctx.dram.put(self._fa_key(ucm_hash), fa_size)
                else:
                    ctx.dram.get(self._fa_key(ucm_hash))
            ...
            if is_hbm_hit or not is_last:
                ...
                if ctx.fs.get(self._wa_key(ucm_hash)):
                    ctx.wa_dram.put(self._wa_key(ucm_hash), wa_size)
                else:
                    ctx.wa_dram.get(self._wa_key(ucm_hash))
    ...
    def _forward_sim(self, record: TraceRecord, ctx: FAWAContext) -> None:
        """Per-chunk loop: WA free (from prev) → FA alloc → WA alloc → dump.

        One ``wa_blocks`` list per group accumulates blocks and is trimmed
        at the start of each chunk: free ``[:-need]`` (non-running), keep
        ``[-need:]`` (running). Sync: trim at next chunk start. Async: full
        list saved to ``pending_prev1`` for 2-chunk delayed free, trimmed to
        running immediately. Last chunk: no next chunk → ``wa_blocks`` stays
        full → ``wa_remaining``.
        """
        vllm_bs = self.topo.vllm_hash_block_size
        ucm_hash_bs = self.topo.ucm_hash_block_size
        ...
        wa_blocks: dict[int, list[tuple[int, bool]]] = {
        ...
        wa_group_map = {g.group_id: g for g in ctx.wa_groups}
        ...
        while pos < total:
            ...
            # 1. Free WA non-running from previous chunk(s)
            if async_delay:
                if pending_prev2:
                    for gid, blocks in pending_prev2.items():
                        g = wa_group_map[gid]
                        need = (
                            ceil((g.sliding_window - 1) / g.logical_block_size)
                            if g.sliding_window
                            else 1
                        )
                        to_free = blocks[:-need] if len(blocks) > need else []
                ...
                for g in ctx.wa_groups:
                    ...
                    blocks = wa_blocks[gid]
                    to_free = blocks[:-need] if len(blocks) > need else []
                    ctx.hbm.free_reverse(to_free)
                    wa_blocks[gid] = blocks[-need:] if len(blocks) >= need else blocks
            ...
            for g in ctx.wa_groups:
                ...
                wa_ce = min(wa_pos[gid] + blocks_per_chunk, wa_total)

                for blk_idx in range(wa_pos[gid], wa_ce):
                    h = g.derive_block_hash(blk_idx, chain)
                    bid = ctx.hbm.alloc()
                    if bid is not None:
                        if g.reachable(blk_idx):
                            ctx.hbm.cache_block(bid, h, g.group_id)
                            self.producer_map[h] = ctx.req_idx
                            wa_blocks[gid].append((bid, True))
                        else:
                            wa_blocks[gid].append((bid, False))

                # Async: save full list for delayed free, trim to running
                if async_delay and not is_last:
                    new_pending[gid] = wa_blocks[gid]
                    wa_blocks[gid] = (
                        wa_blocks[gid][-need:]
                        if len(wa_blocks[gid]) >= need
                        else wa_blocks[gid]
                    )
                # Sync: wa_blocks stays full, trimmed at next chunk start
                # is_last: wa_blocks stays full → wa_remaining

                wa_pos[gid] = wa_ce

            # 4. Roll pending queue (async delay)
            if async_delay:
            ...
            for ucm_idx in range(fa_dumped, ucm_completed):
                ucm_hash = chain[(ucm_idx + 1) * ucm_scale - 1]
                ctx.dram.put(self._fa_key(ucm_hash), fa_size)
                self.producer_map[self._fa_key(ucm_hash)] = ctx.req_idx
                ctx.fs.put(self._fa_key(ucm_hash), fa_size)
                self.producer_map[self._fa_key(ucm_hash)] = ctx.req_idx
            ...
            if wa_block_wise:
                for ucm_idx in range(wa_dumped, ucm_completed):
                    ucm_hash = chain[(ucm_idx + 1) * ucm_scale - 1]
                    ctx.wa_dram.put(self._wa_key(ucm_hash), wa_size)
                    self.producer_map[self._wa_key(ucm_hash)] = ctx.req_idx
                    ctx.fs.put(self._wa_key(ucm_hash), wa_size)
                    self.producer_map[self._wa_key(ucm_hash)] = ctx.req_idx
            else:
        ...
        # 6. Free remaining pending WA (async delay, non-running only)
        if async_delay:
            for pending in (pending_prev2, pending_prev1):
                for gid, blocks in pending.items():
                    g = wa_group_map[gid]
                    need = (
                        ceil((g.sliding_window - 1) / g.logical_block_size)
                        if g.sliding_window
        ...
        ctx.wa_remaining = []
        for g in ctx.wa_groups:
            ctx.wa_remaining.extend(wa_blocks[g.group_id])
    ...
    def _request_end_free(self, ctx: FAWAContext) -> None:
        ...
        # wa_blocks -> freed at next chunk start or survive in wa_remaining).
        # Do NOT free wa_track separately here — that would double-free.
    ...
    def _ucm_fa_lookup(
        ...
        for ucm_idx in range(ucm_start, num_ucm_blocks):
            ucm_hash = chain[(ucm_idx + 1) * ucm_scale - 1]
            if fa_dram.peek(self._fa_key(ucm_hash)):
                ucm_prefix = ucm_idx + 1
                fa_ext_tier = max(fa_ext_tier, 1)
                hit_roots.add(
                    self.request_groups.find(
                        self.producer_map.get(self._fa_key(ucm_hash), req_idx)
                    )
                )
                continue
            if fs.peek(self._fa_key(ucm_hash)):
                ucm_prefix = ucm_idx + 1
                fa_ext_tier = max(fa_ext_tier, 2)
                hit_roots.add(
                    self.request_groups.find(
    ...
    def _ucm_wa_reverse(
        ...
        for ucm_idx in range(upper - 1, -1, -1):
            ...
            if boundary <= skip_below:
                break
            ucm_hash = chain[(ucm_idx + 1) * ucm_scale - 1]
            if ctx.wa_dram.peek(self._wa_key(ucm_hash)):
                ctx.hit_roots.add(
                    self.request_groups.find(
                        self.producer_map.get(self._wa_key(ucm_hash), ctx.req_idx)
                    )
                )
                return boundary, 1
            if ctx.fs.peek(self._wa_key(ucm_hash)):
                ctx.hit_roots.add(
                    self.request_groups.find(
                        self.producer_map.get(self._wa_key(ucm_hash), ctx.req_idx)
                    )
...
class AnalysisRunner:
    ...
    def _build_scenario_defs(
        ...
        return [
            ...
            ("hbm_dram", gpu_cap_blocks, dram_bytes, 0),
            ("hbm_dram_fs", gpu_cap_blocks, dram_bytes, fs_bytes),
        ]
