...
## 📥 Online Profiling
__1.  vLLM + FlashMLA Hook__

Code location:
```python
FlashMLAImpl._forward_decode()
```
Core hook:
```python
flash_mla_with_kvcache(...)
```
Insert tensor-saving logic before the attention computation.

📊 __Collected data__
✅ __Query（Q）__
- Saved from all GPUs (TP ranks)
- One save per decode step
- Stored separately per layer

Filename format：
```python
real_q_{layer_name}_custom{step}.pt
```

✅ __Key Cache（K）__
- Saved only on cuda:0 (to avoid redundancy and storage explosion)
- Full KV cache tensor is stored

Filename format：
```python
real_k_{layer_name}_custom{step}.pt
```
$\quad$

__2. vLLM + Qwen3 Attention Forward Hook__
...
self.attn(...)
```
Insert tensor-saving logic before attention computation.
...
## 📌 Function Overview
__1. K/Q Tensor Loading (Dual-Mode Support)__
✅ __Standard Attention Mode__
```python
enable_mla = False
```
- Each TP rank holds a subset of KV heads
- K/Q organized by __kv head → layer → timestep__

✅ __MLA Mode__
```python
enable_mla = True
```
- Keys are globally shared across all TP ranks
- Queries are split across TP ranks
- Supports dynamically growing decode steps

$\quad$
__2. Sparsity Analysis（Hash Ratio）__
Core workflow:
```yaml
Attention score →
block aggregation →
ranking →
cumulative coverage of top 90% energy
```
Outputs:
- Layer-wise sparsity trajectories across decode timesteps
- Mean statistics across multiple KV heads
- Automatic identification of critical layers for rollback
...
$\quad$
__3. Attention Overlap Matrix__

Used to analyze 
> whether different layers attend to the same token blocks.

Procedure:
- Select top-k attention blocks per layer
- Compute pairwise overlap ratios between layers
- Construct an upper-triangular heatmap

Outputs:
```yaml
matrixX.npy
top{K}Overlap_hammingX.png
```

$\quad$
__4. Automatic Skip-Layer Path Search__
Based on joint optimization over multiple matrices:
- Preserve minimal overlap (no degradation)
- Maximize global attention hit stability
- Limit layer transition frequency (simulate hardware/inference cost)
...
```
Derives:
- Layers eligible for direct attention block reuse
- Layers requiring full recomputation
