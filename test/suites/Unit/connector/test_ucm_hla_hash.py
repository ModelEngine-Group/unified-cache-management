...
def _manager_with_groups(full_attention_block_sizes):
    ...
    manager.base_block_hasher = hasher.make_request_block_hasher(4)
...
def test_base_hash_failure_disables_whole_lookup_without_partial_keys():
    manager, _ = _manager_with_groups([4, 8])

    def failing_hasher(_req):
        raise RequestHashError("simulated base hash failure")

    manager.base_block_hasher = failing_hasher
    with pytest.raises(RequestHashError):
        manager.compute_all_group_block_ids(_request("image"))
