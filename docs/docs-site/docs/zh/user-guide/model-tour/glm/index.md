...
=== "H100"
    ...
    本教程使用预构建 Docker 镜像，在 NVIDIA H100 上通过 vLLM 部署 `GLM-4.7-FP8` 并接入 UCM。

    ## 1. 启动 Docker 容器

    将 `MODEL_DIR` 设为宿主机上存放 `GLM-4.7-FP8` 的父目录（示例模型路径为 `/mnt/model-2/GLM-4.7-FP8`），从[快速开始](../../quick_start/index.md#vllm)复制支持该模型和 H100 的 CUDA **UCM 镜像**地址到 `IMAGE`，在**宿主机**执行：
    ...
    预期返回 HTTP 200，模型列表包含 `glm47`，Chat Completions 响应包含 `choices` 字段。
