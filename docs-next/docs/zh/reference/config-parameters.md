...
| `metrics_config_path` | 选填 | string | 自行配置 | 指定监控指标配置文件路径，启用后可通过 toolkit 进行 UCM 在线监控。Toolkit 使用请参考《Unified Cache 性能调测工具集使用指南》。 |
...
| `storage_backends` | **必填** | string | 自行配置 | 填写创库使用章节中在计算主机的挂载目录。手动配置 nfs over rdma 时多个挂载点需用冒号隔开。 |
...
| 配置项 | 是否必填 | 取值类型 | 取值范围 | 配置说明 |
|---|---|---|---|---|
| `kvcs_store_id` | **必填** | int | — | 填写创库使用章节中存储上已创建的 KV Cache 库 ID。 |
| `kvcs_instance_name` | **必填** | string | 默认 `default_instance` | 推理实例名称，用于异常告警上报区分推理实例，由用户自定义。 |
| `kvcs_ucm_over_tcp_ip_list` | **必填** | string | — | UCM Over TCP 逻辑端口 IP 列表，支持多 IP 以冒号 ":" 分割，如 `192.168.0.1:192.168.0.2`。 |
| `kvcs_block_size` | **必填** | int | 默认 `128` | 与推理引擎 `block_size` 保持一致，拉起 DeepSeek V4 系列模型时填 `4 * block_size`。 |
| `kvcs_tls_enable` | 选填 | bool | 默认 `false` | 推理引擎和存储 GRPC 通信时是否开启 TLS 身份验证。 |
| `kvcs_sliding_window_size` | 选填 | int | 默认 `100`，范围 10-1000 | 滑动窗口大小，指代统计失败率时的推理请求总数（最近 N 次）。 |
| `kvcs_failure_rate_threshold` | 选填 | int | 默认 `10`，范围 0-100 | 推理请求失败率阈值。超过该阈值时隔离 KV Cache 库，所有请求完全重算，上报重要级别告警。 |
| `kvcs_consecutive_fail_limit` | 选填 | int | 默认 `5`，范围 1-100 | 推理请求连续失败次数超过该阈值时隔离 KV Cache 库，所有请求完全重算，上报重要级别告警。 |
