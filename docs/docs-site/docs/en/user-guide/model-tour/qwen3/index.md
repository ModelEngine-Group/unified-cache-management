...
```bash
cat > /data/ucm/ucm_config.yaml <<'YAML'
ucm_connectors:
...
This configuration enables layerwise transfers, storage health checks and metrics. `persist_token_threshold: 0` does not exclude short requests, although reusable data still needs to satisfy cache-block and hybrid-state alignment.
