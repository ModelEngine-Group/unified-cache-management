...
from unifiedcache.ucm_sparse.kvstar.utils import bind_cpus, get_offset
...
class ReqMeta:
    ...
    def query_offload_info(self) -> list | None:
        if self.stage == ReqStage.PREFILL:
            ...
            if (
                cur_step_parse_prompt_len_end_pos
                < self.num_prompt_tokens - RETRIEVAL_TOKEN_GROUP_SIZE
            ):
                return None
            # 计算应该卸载到standby_group的哪些位置
            valid_token_end_pos_in_retrieve_group = RETRIEVAL_TOKEN_GROUP_SIZE - (
...
class ReqPerLayerState:  # 命名风格和vllm保持一致
    ...
    def __init__(
        ...
        self.blk_repre = (
            torch.Tensor()
        )  # NPU: Tensor: (blks, num_key_heads_per_tp, head_size), CPU: list[Tensor], self.full_blk_num, 即CPU: len(self.blk_repre), NPU: self.blk_repre.shape[0]
        ...
        self.standby_query_group: List[Optional[torch.Tensor]] = [
            None
        ] * RETRIEVAL_TOKEN_GROUP_SIZE
        self.do_retrieve_query_group: List[Optional[torch.Tensor]] = [
            None
        ] * RETRIEVAL_TOKEN_GROUP_SIZE
        ...
        self.local_window_sz = (
            LOCAL_WINDOW_SZ  # only save prefill last fully block for acc
        )
    ...
    def retrieval_sync(self, cur_step: int, topk: int, retrieve_device="cpu"):
        ...
        if retrieve_device == "cpu":
            ...
            # queries and key cache block repre
            ...
            if topk >= fully_filed_blk_num_except_fixed_window:
                select_blk_hashes = [
                    f"{self.block_hash(self.req_meta.request_id, id_ + INIT_WINDOW_SZ)}"
                    for id_ in range(0, self.blk_repre.shape[0])
                ]  # 只有prefill阶段的块, 且进行过块裁剪
            else:
                ...
                if self.d_pruned_index is not None:
                    ...
                    for i_h in range(h):
                        q_token_prune[:, i_h, :, :] = q_token[
                            :, i_h, :, self.d_pruned_index[i_h]
                        ]
                ...
                score = torch.nn.functional.softmax(score, dim=-1).to(
                    q_token.dtype
                )  # xhgn
    ...
    def retrieval_async(self, cur_step: int, topk: int, retrieve_device="cpu"):
        ...
        if retrieve_device == "cpu":
            ...
            if topk == 0:
                ...
                return
            retrieve_query_group = self.do_retrieve_query_group
            q_group_stack_tensor = (
                torch.stack(retrieve_query_group)
                .to(torch.float16)
                .contiguous()
                .to("cpu")
            )
    ...
    def attention_begin(
        ...
        if self.req_meta.stage == ReqStage.PREFILL:
            ...
            if chunk_prefill_query_offload_info:
                ...
                for query_relative_idx, in_query_group_idx in enumerate(
                    ...
                    self.standby_query_group[in_query_group_idx] = tokens_to_offload[
                        query_relative_idx
                    ]
            ...
            self.standby_query_group[step_idx_in_retrieve_group] = query[
                query_start_loc
            ]
        ...
        if self.req_meta.step % RETRIEVAL_TOKEN_GROUP_SIZE == 1:
            ...
            if self.blk_repre is not None:
                candidate_swap_vllm_block_ids = self.req_meta.vllm_block_ids[
                    self.init_window_sz : math.ceil(
                        self.blk_repre.shape[0] * SPARSE_RATIO
                    )
                    + self.init_window_sz
                ]
            ...
            if self.req_meta.step <= RETRIEVAL_TOKEN_GROUP_SIZE * 2:
                ...
                cur_group_idx = int(
                    math.ceil(self.req_meta.step / RETRIEVAL_TOKEN_GROUP_SIZE)
                )  # e.g. step 17 / 8 = 第3组
                wait_retrieve_step_idx = (
                    cur_group_idx - 3
                ) * RETRIEVAL_TOKEN_GROUP_SIZE + 1
            ...
            if self.step_group_retrieve_result.get(need_retrieve_record) is None:
                ...
                if task_result["status"] == "SUCCESS":  # 假设 0 代表 SUCCESS
                    ...
                    select_blk_hashes = [
                        f"{self.block_hash(self.req_meta.request_id, int(id_) + INIT_WINDOW_SZ)}"
                        for id_ in topk_indices
                    ]
            ...
            # -------------------------触发块加载---------------------------------
            if (
                need_retrieve_record != "prefill" or self.req_meta.step == 1
            ):  # prefill的检索结果可以被decode step 1~16用
    ...
    def save_blocks(self, num_blocks_need_dump):
        ...
        )  # prefill dense token blk, decode sparse budget, must finish prefill phase task before schedule req to decode
        # num_blocks_dumped = self.blk_repre.shape[0] # NOTE: 这里无法通过self.blk_repre获取dump块数
        cur_dumped_blk_num = self.num_blocks_dumped
        need_save_blk_ids = list(
            range(cur_dumped_blk_num, cur_dumped_blk_num + num_blocks_need_dump)
        )
        need_save_block_hashes = [
            f"{self.block_hash(self.req_meta.request_id, id_)}"
            for id_ in need_save_blk_ids
        ]

        if self.req_meta.stage == ReqStage.PREFILL:
            vllm_block_ids_dump = vllm_block_ids[
                cur_dumped_blk_num : cur_dumped_blk_num + num_blocks_need_dump
            ]
        else:  # 当前不支持投机, decode只能卸载尾部满块 TODO: 支持投机
            vllm_block_ids_dump = vllm_block_ids[-1:]
        self.launch_transfer_task(
            "dump", need_save_block_hashes, vllm_block_ids_dump
        )  # 先异步卸载块, 再计算表征, 掩盖时延
        if self.req_meta.stage == ReqStage.PREFILL and self.req_meta.is_last_chunk:
            self.blk_repre = self.extract_block_repre(
                vllm_block_ids_dump, prune_dim_enable=True
            )
            # NOTE: 关键, 维度剔除首尾块
            if self.blk_repre is not None:
...
class KVStarMultiStep(UcmSparseBase):
    ...
    def wait_all_task_done(
        self,
    ):  # 一些异步任务等待逻辑 NOTE: 注意区分检索任务和blk传输任务, 这里是multistep策略worker侧整体的wait_all, 注意区分req_layerwise级别的wait_for
        pass
    ...
    def estimate_num_slots_sparsed(self, request: Request) -> int:
        ...
        num_prefill_keep_fixed_blk = min(
            INIT_WINDOW_SZ + LOCAL_WINDOW_SZ, num_prefill_fully_block
        )

        num_sparse_saved_fully_blk = math.ceil(
            (num_prefill_fully_block - num_prefill_keep_fixed_blk) * SPARSE_RATIO
