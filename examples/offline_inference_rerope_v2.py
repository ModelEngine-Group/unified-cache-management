...
def setup_environment_variables():
    ...
    os.environ["VLLM_EXECUTE_MODEL_TIMEOUT_SECONDS"] = "60000"
    ...
    path_to_dataset = os.getenv("DATASET_FILE", "/home/externals/wangwenxin21/datasets/longbench-v2/filtered_128k_1M_data.jsonl")
    if path_to_dataset and not os.path.isfile(path_to_dataset):
        path_to_dataset = None

    data_dir = os.getenv("STORAGE_BACKENDS", "/home/externals/wangwenxin21/wx_data")
...
def format_prompt_for_model(prompt: str, use_chat_template: bool = True, truncate: bool = True) -> str:
    ...
    try:
        ...
        # This is similar to pred.py: take first half and last half
...
def main():
    ...
    if args.cot:
        out_file = os.path.join(args.save_dir, f"{model_name}_longbench_v2{domain_suffix}_cot_ucm_sparse_config_esa_1_2_0.3_5.jsonl")
    elif args.no_context:
        out_file = os.path.join(args.save_dir, f"{model_name}_longbench_v2{domain_suffix}_no_context.jsonl")
    else:
        out_file = os.path.join(args.save_dir, f"{model_name}_longbench_v2{domain_suffix}.jsonl")
