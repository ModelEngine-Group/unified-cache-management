...
class TestModelValidator:
    ...
    def test_model_validate_naive(self, model_config: ModelConfig) -> Dict[str, Any]:
        ...
        accuracy = self._run_accuracy_test(model_config, "Naive")['metric_dict']['f1-score']
    ...
    def test_model_validate_pc(self, model_config: ModelConfig) -> None:
        ...
        accuracy = self._run_accuracy_test(model_config, "PC")['metric_dict']['f1-score']

        # Fetch baseline (naive) results for comparison
        naive_dict = self._fetch_naive_result()
        if naive_dict:
            perf_metrics.insert(0, {
                "hit_rate": "naive",
    ...
    def test_model_validate_sparse(self, model_config: ModelConfig) -> None:
        ...
        perf_metrics = self._run_perf_test(hit_rates=[0])
        accuracy = self._run_accuracy_test(model_config, "Sparse")['metric_dict']['f1-score']
    ...
    def _fetch_naive_result(self) -> Optional[Dict[str, Any]]:
        records = read_from_db("model_validate_test", {"config_key": _make_config_key()}, limit=1)
    ...
    def _print_perf_summary(self, results: List[Dict[str, Any]]) -> None:
        ...
        for r in results:
            print(f"{HIGHLIGHT}{r['hit_rate']!s:<12} {MEAN_INPUT_TOKENS:<15} {MEAN_OUTPUT_TOKENS:<15} {CONCURRENT_REQUESTS:<12} {r['ttft_mean']:<20.4f} {r['tpot_mean']:<20.4f} {r['e2e_mean']:<20.4f}{RESET}")
        print(f"{HIGHLIGHT}{'=' * 110}{RESET}")

    def _print_accuracy_comparison(self, naive_val: Optional[float], accuracy_val: float, test_id: str) -> None:
        HIGHLIGHT = '\033[1;96m'
        RESET = '\033[0m'
        print(f"\n{HIGHLIGHT}{'=' * 40}{RESET}")
