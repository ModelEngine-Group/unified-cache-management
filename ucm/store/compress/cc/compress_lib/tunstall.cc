...
#define TS_V2_EXPAND             1                         // 使用 V2 版算法来进行 tunstall LUT 展开 。当剩余 prob 小于阈值时停止展开低概率符号，避免浪费LUT条目
#ifndef TS_V2_THRESH
...
#define  RET_WHEN_ERROR(err_code)          {                      int e = (err_code); if (e) {return e;} }
...
static size_t ts_get_header_length (const ts_header_t *p_hdr) {
    ...
    if        (mode == TS_MODE_UNCOMPRESS) {
        ...
        return sizeof(p_hdr->mode) + sizeof(p_hdr->n_symb) +
               sizeof(p_hdr->n_mark) + sizeof(p_hdr->src_len) +
               p_hdr->n_symb;
    } else {
        return (size_t)(-1);
    }
...
// 功能：不压缩直接存数据
static int ts_do_not_compress (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len) {
    uint8_t *p_dst_base = p_dst;                                                           // 整个压缩流的起始地址 
    ts_header_t *p_hdr = (ts_header_t*)p_dst;                                              // 压缩流头直接指向压缩流的开始，后续构建 p_hdr ，避免搬移
    RET_ERROR_IF(R_ERR_DST_OVERFLOW, (src_len + sizeof(p_hdr->uncompress) > *p_dst_len));  // 如果dst塞不下，报错
...
static void ts_build_idx2symb_symb2idx (uint8_t *p_idx2symb, uint8_t *p_symb2idx, const ts_hist_t *p_hist) {
    for (int idx = 0; idx < p_hist->n_symb; idx++) {  // 这里 symb 循环变量类型要取 int 而不uint8_t ，避免死循环
...
static int ts_init_predef_probs (ts_hist_t *p_hist, int n_symb, int lambda) {
    ...
    for (int idx=0; idx<n_symb; idx++) {
        ...
        p_hist->hist[idx].prob = exp(- ((lambda + 1.0)/64.0) * idx);   // 函数表达式f[x] = exp (- ((λ+1)/64) * x)
    }
    double sum = 0.0;
    ...
    for (int idx=0; idx<n_symb; idx++) {                                   // 归一化
        ...
        p_hist->hist[idx].log_prob = log(p_hist->hist[idx].prob) * neg_inv_loge2; // log_prob = log_e(1.0 / prob) / log_e(2.0);
    }
    return R_TS_OK;
}


// 功能：计算待压缩的数据的实际分布 (p_hist) 在某个预设LUT (p_pdlut) 下的交叉熵，交叉熵越小，说明越适合用这个预设表来压缩
static double ts_cross_entropy (const ts_hist_t *p_hist1, const ts_hist_t *p_hist2) {
    if (p_hist1->n_symb > p_hist2->n_symb) {   // 实际数据的取值范围> 预设LUT的取值范围
        ...
        for (int idx=0; idx<p_hist1->n_symb; idx++) {
            entropy += p_hist1->hist[idx].prob * p_hist2->hist[idx].log_prob;  // 交叉熵= p1 * log2(1.0 / p2)
        }
        return entropy;
    }
}
...
static int ts_get_hist_from_array (ts_hist_t *p_hist, const uint8_t *p_src, size_t src_len) {
    ...
    for (int symb = 0; symb < TS_N_SYMB; symb++) {         // 循环：遍历所有符号，把频率freq转化为概率prob 这里 symb 循环变量类型要取 int 而不uint8_t ，避免死循环
    ...
    qsort((void*)(p_hist->hist), TS_N_SYMB, sizeof(ts_hist_item_t), ts_cmp_hist); // hist 排序, 频率高的排在前面
    return R_TS_OK;
}
...
static uint32_t ts_search_largest_item (const ts_lut_item_t *p_lut, const double *p_lut_prob, const uint8_t *p_black_list, uint32_t lut_size) {
...
static uint16_t ts_goto_state_by_lut_item (const ts_enc_state_item_t *p_enc_table, const ts_lut_item_t *p_item) {
    uint16_t state = 0;
...
//       每次LUT 中删除概率最大的 item, 并遍历所有非symb , 给这里item 末尾拼上这个 symb 作为最新的一item 加入 LUT 
//       顺便也会建立状态转移树p_enc_table , 用来压缩
static int ts_build_lut (
    ts_lut_item_t       *p_lut,
    ...
    uint8_t lut_black_list [TS_LUT_SIZE] = {0};        // 数组：黑名单，上了黑名单的LUT表项不能被展开，初始化全都不在黑名
    ...
    do {
        uint32_t mark = ts_search_largest_item(p_lut, lut_prob, lut_black_list, lut_size);  // 找概率最大的可展开项。并排除黑名
        ...
        uint16_t state = ts_goto_state_by_lut_item(p_enc_table, &max_item); // 找到 max_item (也即需要展开的item) 对应的state：沿着一ts_lut_item_t 中包含的字符，从 p_enc_table 的根节点摸到一个节
        ...
        for (int idx = 0; idx < n_symb; idx++) {                    // 遍历所symb, 逐个展开
            ...
            if (mark < TS_LUT_SIZE-1) {                                             // 如果 LUT 还没被填
                #if TS_V2_EXPAND                                                    // 是否使用 V2 版算法来进行 tunstall LUT 展开 。当剩余 prob 小于阈值时停止展开低概率符号，避免浪费LUT条目
                    if (max_item.c > 0) {                                           // V2关键特 "中断展开" 注意: 只有非根节点允许"中断展开", 所以这里要判断 (max_item.c > 0)
                        if (0 < idx && idx < (n_symb - 1)) {                        // 待展开symb 不是第一个，也不是最后一
                            if (max_prob*rem_prob < (TS_V2_THRESH / TS_LUT_SIZE)) { // 剩余未展开symb 的总概率是否小于一个阈值？
                ...
                new_state ++;                                       // 状态转移表+1
                p_enc_table[state].next_state[symb] = new_state;    // 建立状态转state(symb) -> new_state
                p_enc_table[new_state].mark = mark;

            } else if (idx >= (n_symb - 1)) {                       // 如果 LUT 被填(当前 mark 已经对应最后一个表项了) , 而且当前面item 刚好被展开完了
                p_lut[mark] = ts_add_symb_to_item(&max_item, symb); // 添加新符号创建新
                new_state ++;                                       // 状态转移表+1
                p_enc_table[state].next_state[symb] = new_state;    // 建立状态转state(symb) -> new_state
                p_enc_table[new_state].mark = mark;
                break;               
            } else {                                                // 如果 LUT 被填(当前 mark 已经对应最后一个表项了) , 但是当前面item 没被完全展开
...
int TunstallInitAllPredefTables (TunstallPredefTables_t *p_predef_luts) {
    for (int lambda = 0; lambda < TS_COUNT_PREDEF; lambda++) {
        RET_WHEN_ERROR(ts_init_predef_probs(&(p_predef_luts->hist[lambda]), TS_N_SYMB_PREDEF, lambda));
        RET_WHEN_ERROR(ts_build_lut(p_predef_luts->lut[lambda], p_predef_luts->etree[lambda], &(p_predef_luts->hist[lambda])));
        RET_WHEN_ERROR(ts_check_lut(p_predef_luts->lut[lambda]));  // 检查刚建立LUT 表是否符合一些基本要
    }
...
static int ts_find_best (const TunstallPredefTables_t *p_predef_luts, const ts_hist_t *p_hist, uint8_t *p_best_lambda) {
...
static void ts_cvt_lut_idx2symb (ts_lut_item_t *p_lut_dst, const ts_lut_item_t *p_lut_src, const uint8_t *p_idx2symb) {
...
static void ts_cvt_data_idx2symb (uint8_t *p_data, size_t src_len, const uint8_t *p_idx2symb) {
    for (size_t i=0; i<src_len; i++) {
        p_data[i] = p_idx2symb[ p_data[i] ];
    }
}


// 功能：沿着 p_src 的字符，p_enc_table 的根节点摸到一个叶节点，返回叶节点mark
static uint16_t ts_get_mark_by_input_stream (const ts_enc_state_item_t *p_enc_table, const uint8_t **ipp, const uint8_t *ip_end, uint8_t most_freq_symb) {
    uint16_t state = 0;
    for (;;) {
...
static int ts_encode_dynamic (const ts_enc_state_item_t *p_enc_table, uint8_t **opp, uint8_t *op_end, const uint8_t *ip, size_t src_len, uint8_t most_freq_symb, uint32_t *p_n_mark) {
...
// 功能：沿着 p_src 的字符，p_enc_table 的根节点摸到一个叶节点，返回叶节点mark
static uint16_t ts_get_mark_by_input_stream_symb2idx (const ts_enc_state_item_t *p_enc_table, const uint8_t *p_symb2idx, const uint8_t **ipp, const uint8_t *ip_end) {
    uint16_t state = 0;
    for (;;) {
        uint8_t symb = 0;
        if (*ipp < ip_end) {
...
// 功能：使用构建好LUT 表对输入数据进行 Tunstall 编码。每次查找最长匹配，将两个标记打包输
static int ts_encode_predef (const ts_enc_state_item_t *p_enc_table, const uint8_t *p_symb2idx, uint8_t **opp, uint8_t *op_end, const uint8_t *ip, size_t src_len, uint32_t *p_n_mark) {
    const uint8_t *ip_end = ip + src_len;
    *p_n_mark = 0;
...
int TunstallCompressDynamic (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len) {
    ...
    ts_enc_state_item_t etree [TS_ENC_STATE_TABLE_SIZE];                    // TODO : TS_N_SYMB 太大小 etree 占空间太大，栈放不下
    RET_WHEN_ERROR(ts_build_lut(p_hdr->dynamic.lut, etree, &hist));                 // 建立 LUT, 直接建立p_hdr->dynamic.lut 
    ...
    RET_WHEN_ERROR(ts_encode_dynamic(etree, &p_dst, (p_dst_base+*p_dst_len), p_src, src_len, most_freq_symb, &p_hdr->dynamic.n_mark)); // 执行 tunstall 编码
...
// 功能：Tunstall压缩主函数。构建直方图、生成LUT表、执行编码，返回压缩后的数据
int TunstallCompressPredef (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len, const TunstallPredefTables_t *p_predef_luts) {
    RET_ERROR_IF(R_ERR_PREDEF_UNINIT, !ts_all_predef_luts_available(p_predef_luts));
    RET_ERROR_IF(R_ERR_UNSUPPORT, TS_MARK_BITS != 8 && TS_MARK_BITS != 12);     // 参数检
    RET_ERROR_IF(R_ERR_DST_OVERFLOW, *p_dst_len < TS_LUT_BYTES);                // 目标缓冲区必须能容纳LUT
    RET_ERROR_IF(R_ERR_SRC_OVERFLOW, src_len == 0 || src_len > 0xFFFFFFFF);     // 源缓冲区长度检
    ...
    ts_hist_t hist;                                                             // 统计信息
    RET_WHEN_ERROR(ts_get_hist_from_array(&hist, p_src, src_len));              // 统计符号频率, 得到直方图
    RET_ERROR_IF(R_ERR_SYMB_RANGE_PREDEF, hist.n_symb > TS_N_SYMB_PREDEF);      // 如果非零符号数量 > 预设表模式所允许的最大非零符号数据
    ...
    RET_WHEN_ERROR(ts_encode_predef(p_predef_luts->etree[lambda], symb2idx, &p_dst, (p_dst_base+*p_dst_len), p_src, src_len, &p_hdr->n_mark)); // 执行 tunstall 编码
...
int TunstallCompress (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len, const TunstallPredefTables_t *p_predef_luts) {
    ...
    int err_predef = TunstallCompressPredef(p_dst, &len_predef, p_src, src_len, p_predef_luts); // 尝试predef 压缩
    ...
    if (!err_dynam) {                                                              // 最终选择一种模式进行压缩
        ...
        *p_dst_len = len_predef;                                                   // 采用 predef 模式，这里不需要再压缩了，因为 dst 里面已经放了 predef 压缩的数据
        return R_TS_OK;                                                               //
    } else {                                                                       //
...
int TunstallDecompress (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, int do_idx2symb_on_predef_lut, const TunstallPredefTables_t *p_predef_luts) {
