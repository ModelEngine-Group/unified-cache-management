...
class GSAReqStat:
    ...
    def set_block_hashes(self, token_ids):
        ...
        for start in range(0, len(token_ids), self.block_size):
            ...
            block_hash = block_hash_func(parent_block_hash_value, curr_block_token_ids_tuple)
    ...
    def _get_sparse_and_free_block(self):
        ...
        if self.num_prompt_tokens > SEG_PREFILL_THRESHOLD and PTOPK_PREFETCH_ENABLE:
            ...
            if remain_len < blocks_len:
                ...
                self.remain_idx = (req_idx_list[:init_windows_size] +
                                   req_idx_list[init_windows_size - remain_len:])
                self.prefetch_idx = req_idx_list[init_windows_size - remain_len - prefetch_len:init_windows_size - remain_len]
...
class GSA(UcmSparseBase):
    ...
    def attention_begin(
        ...
        if self.prefetch_engine.atb_gsa_enable:
            if not self.use_mla:
                ...
                if phase == "decode":
                    if torch.cuda.is_available():
                        attn_metadata.decode.block_table = self.model_input["block_tables_mp"][
                            current_layer_id
                        ][self.decode_index]
                        ...
                        attn_metadata.decode.block_tables[
                            : len(self.prefetch_engine.req_ids_bs)
                        ].copy_(self.model_input["block_tables_mp"][current_layer_id][self.decode_index])
                        attn_metadata.decode.seq_lens.copy_(
                            self.model_input["gsa_seq_len"][current_layer_id][self.decode_index]
                        )
    ...
    def attention_finished(
        ...
        for req_id in self.prefetch_engine.req_ids_bs:
            ...
            req_meta = self.gsa_metadata.gsa_stats[req_id]
            if (
                req_meta.is_last_chunk() and
                req_meta.num_prompt_tokens > SEG_PREFILL_THRESHOLD and
                PTOPK_PREFETCH_ENABLE
            ):
                blocks_len = len(self.gsa_metadata.gsa_stats[req_id].blocks)
                remain_len = gsa_config.compute_topk_len(blocks_len)
                prefetch_len = min(gsa_config.num_prefetch_blocks, blocks_len - remain_len)
                topk_value = self.last_chunk_topk_cal(req_meta,
                                                      query,
                                                      current_layer_id,
                                                      remain_len + prefetch_len)

                if self.gsa_metadata.gsa_stats[req_id].reamin_map == None:
                    self.gsa_metadata.gsa_stats[req_id].reamin_map = [None] * self.layer_num
                    self.gsa_metadata.gsa_stats[req_id].prefetch_map = [None] * self.layer_num
                ...
                if self.gsa_metadata.gsa_stats[req_id].topk_buf_tmp == None:
                    self.gsa_metadata.gsa_stats[req_id].topk_buf_tmp = (
                        torch.zeros((self.layer_num, len(topk_value)), dtype=torch.int32, device="cpu")
                    )
                self.gsa_metadata.gsa_stats[req_id].topk_buf_tmp[current_layer_id] = topk_value
    ...
    def last_chunk_topk_cal(self, req_meta, query, current_layer_id, first_topk_len):
        ...
        if not self.use_mla:
            ...
            cal_topk_id = [self.model_input["query_locals_prefill"][index_in_batch + 1] - 1]
        head_group_num = self.att_num_heads // self.num_key_heads
        q_decode = query[cal_topk_id]

        include_mask = torch.tensor(req_meta.include_mask, dtype=torch.uint8, device=self.device)
        exclude_mask = torch.tensor(req_meta.exclude_mask, dtype=torch.uint8, device=self.device)
        if CUDA_TOPK:
            kpre_index = torch.tensor(req_meta.repre_slot_mapping, dtype=torch.int32, device=self.device)
            kpre_need = self.prefetch_engine.kpre_caches[current_layer_id][kpre_index]
        else:
            kpre_index = torch.tensor(req_meta.repre_slot_mapping, dtype=torch.int32, device="cpu")
            kpre_need = self.prefetch_engine.kpre_caches[current_layer_id][kpre_index].to(device=self.device, dtype=self.dtype)
        ...
        dot_product_weights = attention_weights_without_norm.mean(1)
        dot_product_weights.masked_fill_(
            include_mask == 1, float("inf")
        )
        dot_product_weights.masked_fill_(
            exclude_mask == 1, float("-inf")
        )
        _, top_indices = torch.topk(
            dot_product_weights, first_topk_len, dim=-1
        )
        return top_indices[0].cpu()

    
    def kvcache_init_last_chunk(self, forward_context: ForwardContext, layer_name, topk_value, req_id):
        current_layer_id = int(layer_name.split(".")[2])
        blocks_len = len(self.gsa_metadata.gsa_stats[req_id].blocks)
        remain_len = gsa_config.compute_topk_len(blocks_len)
        prefetch_len = min(gsa_config.num_prefetch_blocks, blocks_len - remain_len)
        req_idx_list = list(range(blocks_len))
        init_windows_size = gsa_config.init_windows_size
        remain_idx = (req_idx_list[:init_windows_size] +
                      req_idx_list[init_windows_size - remain_len - prefetch_len:])
        assert len(remain_idx) == len(topk_value)
        mv_map, reamin_map, prefetch_map = self.get_mv_map(self.gsa_metadata.gsa_stats[req_id].blocks,
                                                           remain_idx,
                                                           topk_value.tolist(),
                                                           remain_len)
    ...
    def build_sparse_meta(
        ...
        self.decode_index = [input_batch.req_id_to_index[rid] for rid in req_ids if num_sched.get(rid, 0) == 1]
    ...
    def _start_topk_cal(self) -> None:
        ...
        for req_id in self.prefetch_engine.req_ids_bs:
            ...
            if req_meta.is_gsa():
                ...
                one_topk_len = gsa_config.compute_topk_len(len(req_meta.blocks)) + gsa_config.num_prefetch_blocks
    ...
    def allocate_slots(self, request, num_slots_sparsed, coordinator, block_pool, kv_cache_groups):
        if request.num_prompt_tokens + 1 == request.num_tokens and request.num_tokens % self.block_size == 1:
        ...
        for i, block in enumerate(allocated_blocks):
            if (i >= num_blocks_original - num_blocks_need + init_windows_size
        ...
        coordinator.single_type_managers[0].req_to_blocks[request.request_id] = kept_blocks

        new_computed_block_list = tuple(
            [] for _ in range(len(kv_cache_groups)))
