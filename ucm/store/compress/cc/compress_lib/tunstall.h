...
typedef struct {
    ...
    double   log_prob; // log_probs = log2(1.0 / prob) , 提前计算好放在这里 用来快速计算预设表和待压缩数据的交叉熵
} ts_hist_item_t;

// 对输入数据的统计直方图
typedef struct {
    ts_hist_item_t hist [TS_N_SYMB]; // 直方图，其中的每项是 {symb, freq, prob} 按照 freq 从大到小排序
...
#define TS_ENC_STATE_TABLE_BYTES (TS_ENC_STATE_TABLE_SIZE * sizeof(ts_enc_state_item_t)) // enc table 最大的大小 (Bytes)
...
extern int TunstallCompressDynamic (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len);
extern int TunstallCompressPredef  (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len               , const TunstallPredefTables_t *p_predef_luts);
extern int TunstallCompress        (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, size_t src_len               , const TunstallPredefTables_t *p_predef_luts);
extern int TunstallDecompress      (uint8_t *p_dst, size_t *p_dst_len, const uint8_t *p_src, int do_idx2symb_on_predef_lut, const TunstallPredefTables_t *p_predef_luts);
