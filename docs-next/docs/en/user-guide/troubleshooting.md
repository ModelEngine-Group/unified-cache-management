...
2. Verify parameter names match the [Pipeline Store](capabilities/prefix-cache/pipeline-store.md) or [NFS Store](capabilities/prefix-cache/nfs-store.md) documentation
...
**Solution**: UCM currently supports CUDA and Ascend platforms only. Check the [Support Matrix](../reference/compatibility.md) for details.
