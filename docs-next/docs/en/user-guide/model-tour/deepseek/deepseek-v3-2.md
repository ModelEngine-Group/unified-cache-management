...
DeepSeek-V3.2 (MLA cache layout).
...
vllm serve /models/DeepSeek-V3.2 \
  --max-model-len 131072 \
  --tensor-parallel-size 8 \
...
For multi-node PD disaggregation, see the [GLM-5.1 4-node PD guide](../glm/glm-5-1.md) for a complete worked example with Mooncake + UCM MultiConnector.

## UCM Configuration

`ucm_config_example.yaml` (MLA, ~24 GB/card):
...
curl http://127.0.0.1:7800/v1/chat/completions \
  ...
  -d '{"model":"deepseek-ai/DeepSeek-V3.2","messages":[{"role":"user","content":"hi"}],"max_tokens":32}'
```
