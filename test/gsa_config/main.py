...
class RALS:
    def __init__(self, args, reuse=False):
        self.args = args
        self.savepath = f'./output/{os.path.basename(args.datapath)}/{args.calibration_data}'
    ...
    def KQloader(self):
        ...
        if self.args.enable_mla:
            ...
            max_file = max(
                files,
                key=lambda p: int(p.stem.split("attn_custom")[-1])
            )
            ...
            for layer in tqdm.tqdm(range(self.args.layers), desc="Loading MLA tensors"):
                # Load K (只在 TP0)
                k_path = f"{root}/{prefix_k}{layer}.self_attn.attn_custom{self.max_step}.pt"
                ...
                for tp in range(self.args.TP):
                    tp_root = root.replace(f"{self.args.device}0", f"{self.args.device}{tp}")
            ...
            self.max_step = self.TQ[self.args.kvheads - 1][self.args.layers - 1].shape[0] - 1
    ...
    def getoverlapMatrix(self, perserved_tokens):
        ...
        for head in range(first_loop):
            ...
            for step in tqdm.tqdm(range(count)):
                ...
                for layer in range(self.args.layers):
                    # ----------- Load K -----------
                    k = self.TK[0][layer] if self.args.enable_mla else self.TK[head][layer]
                    k = k[:k.shape[0] - self.max_step + step]
                    k = k.reshape(k.shape[0], -1, self.args.dim)
            ...
            plt.tight_layout()
            plt.savefig(Path(self.savepath) / f"top{perserved_tokens}_overlap_{head}.png", dpi=300)
            plt.close()
    ...
    def getSkipLayerConfig(self, rollbackLayers, sparsity, computed_ratio_of_hamming_layers):
        """
        ...
        # =====================================================
        max_changes = (
            math.ceil(self.args.layers * sparsity * computed_ratio_of_hamming_layers)
            + len(rollbackLayers)
        )
        ...
        for idx, Table in enumerate(TABLELIST):
            ...
            plt.title(
                f'Rollback={rollbackLayers}, '
                f'Computed_hammings={hamming_count}'
            )
            plt.tight_layout()
    ...
    def run(self, 
        ...
        self.getSkipLayerConfig(rollbackLayers, sparsity, computed_ratio_of_hamming_layers)
