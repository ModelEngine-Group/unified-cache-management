...
- [Prefix Cache](../capabilities/prefix-cache/index.md)
- [GSA Sparsity](../capabilities/sparse-attention/gsa.md)

For quick start, just follow the guide below to launch your own inference experience;

### Feature 1:  Prefix Caching

You may directly edit the example file at `unified-cache-management/examples/ucm_config_example.yaml`. For more please refer to [Prefix Cache with NFS Store](../capabilities/prefix-cache/nfs-store.md) and [Prefix Cache with Pipeline Store](../capabilities/prefix-cache/pipeline-store.md) document.

Make sure to replace `/mnt/test` with your actual storage directory. 
...
```
**The parameter `--no-enable-prefix-caching` is for SSD performance testing, please remove it for production.**

**Make sure to replace `"/workspace/unified-cache-management/examples/ucm_config_example.yaml"` with your actual config file path.**

**The log files of UCM module will be put under `log` directory of the path you start vllm service. To use a custom log path, set `export UCM_LOG_PATH=my_log_dir`.**
