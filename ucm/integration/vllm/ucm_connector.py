...
class KVCacheLayout:
    def __init__(
        ...
        self.use_layer_shards = self.use_layerwise or bool(
            ucm_config.get("direct_submit_by_layer", False)
        )
...
class UCMDirectConnector(KVConnectorBase_V1):
    ...
    def __init__(
        ...
        logger.info(
            ...
            f"blocks_per_chunk={self.blocks_per_chunk}, "
            f"direct_submit_by_layer={self.direct_submit_by_layer}"
        )
    ...
    def _submit_direct_layer_load_tasks(
        self,
        ucm_block_ids: list[bytes],
        vllm_block_ids: list[int],
    ) -> list[Task]:
    ...
    def _submit_direct_layer_dump_tasks(
        ...
        for local_layer_id, layer_id in enumerate(self.layer_ids):
            if not self._submit_direct_layer_dump_task(
                layer_id,
                local_layer_id,
                total_ucm_block_ids,
                total_ptrs,
                dump_request_ids,
                wait_for_save_start_ms,
            ):
                return False

        return True

    def _submit_direct_layer_dump_task(
        self,
        layer_id: int,
        local_layer_id: int,
        total_ucm_block_ids: list[bytes],
        ...
        try:
            ...
            logger.error(
                f"submit layer dump task for layer {layer_id} failed. "
                f"{type(e).__name__}: {e}"
            )
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        request_to_tasks: dict[str, list[Task]] = {}
        is_load = False
        num_loaded_block = 0
        ...
        for request_id, request in metadata.request_meta.items():
            if len(request.load_block_ids[0]) == 0:
                continue
            is_load = True
            request_load_blocks = len(request.load_block_ids[0]) * self.blocks_per_chunk
            ...
            try:
                if self.direct_submit_by_layer:
                    tasks = self._submit_direct_layer_load_tasks(
                        ucm_block_ids,
                        vllm_block_ids,
                    )
                else:
                    ...
                    tasks = [
                        self.store.load_data(ucm_block_ids, shard_indexs, total_ptrs)
                    ]
                request_to_tasks[request_id] = tasks
    ...
    def wait_for_save(self) -> None:
        ...
        if is_save:
            if self.direct_submit_by_layer:
                if not self._submit_direct_layer_dump_tasks(
                    total_ucm_block_ids,
                    total_vllm_block_ids,
                    dump_request_ids,
                    wait_for_save_start_ms,
...
class UCMLayerWiseConnector(UCMDirectConnector):
    ...
    def _submit_layerwise_dump_task(
        ...
        self._submit_direct_layer_dump_task(
            layer_id,
            local_layer_id,
            total_ucm_block_ids,
            self.dump_total_ptrs,
            dump_request_ids,
        )
    ...
    def _submit_request_load_tasks_for_layer(
        ...
        for request_id, ucm_block_ids, total_ptrs in self.request_data:
            if request_id in self._failure_req_ids:
                continue
            try:
                task = self._submit_direct_layer_load_task(
                    layer_id,
                    local_row,
                    ucm_block_ids,
                    total_ptrs,
